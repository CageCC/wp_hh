...
function interface_footer_infoblog( $set_flags, $place ='') {
	...
	if($set_flags || (!empty($options['social_phone'] ) || !empty($options['social_email'] ) || !empty($options['social_location']))){
		...
		$interface_footer_infoblog .=  '<div class="info-bar"><div class="container clearfix">';

		if(!empty($options['social_phone'] ) || !empty($options['social_email'] ) || !empty($options['social_location']) ) {
			$interface_footer_infoblog .=  '<div class="info clearfix"><ul>';
			...
			$interface_footer_infoblog .= '</ul></div><!-- .info -->';
		}
...
function interface_socialnetworks( $set_flags ) {
	...
	if ( ( 1 != $set_flags ) || ( 1 == $set_flags ) )  {
		...
		if($i > 0) {
			$interface_socialnetworks .='<div class="social-profiles clearfix"><ul>';
			$interface_socialnetworks .= $a;
						
		
			$interface_socialnetworks .='</ul></div><!-- .social-profiles -->';
		}	
...
/** end interface_headercontent_details */
	...
	if( !$interface_featured_sliders != empty( $options[ 'featured_post_slider' ] ) ) {
		...
		while ( $get_featured_posts->have_posts()) : 
			...
			if( $title_attribute != '' || $excerpt !='' ) {
				...
				$interface_featured_sliders .= '</article><!-- .featured-text --></div>';
			}
...
/** end interface_featured_sliders  **/
